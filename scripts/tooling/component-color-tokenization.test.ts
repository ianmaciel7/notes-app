...
const componentRoots = [join(sourceRoot, "components"), join(sourceRoot, "app", "_components")];
...
it("keeps component color values in global CSS tokens", () => {
  ...
  const offenders = componentRoots.flatMap((componentRoot) =>
    listComponentSources(componentRoot).flatMap((filePath) => {
