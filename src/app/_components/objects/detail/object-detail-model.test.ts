...
it("preserves HTTP(S) links and the legacy trailing-slash normalization", () => {
  expect(getWeblinkUrl(objectEntityFixture({ properties: { url: "https://example.com/" } }))).toBe(
    "https://example.com",
  );
  expect(getWeblinkUrl(objectEntityFixture({ properties: { href: "http://example.com/a" } }))).toBe(
    "http://example.com/a",
  );
});
...
it("keeps false and zero properties and hides URL keys case-insensitively", () => {
  ...
  expect(getReadableProperties(entity, ["url"])).toEqual([
    ["complete", "false"],
    ["count", "0"],
  ]);
});
