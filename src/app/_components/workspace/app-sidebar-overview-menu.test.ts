...
it("renders sidebar action menus with Capacities compact rows and framed icons", () => {
  const componentSource = readSource("app/_components/workspace/app-sidebar-overview.tsx");
...
it("makes pinned and object type rows draggable without adding visible handles", () => {
  const componentSource = readSource("app/_components/workspace/app-sidebar-overview.tsx");
  const paritySource = readSource("app/_components/workspace/app-sidebar-overview-parity.tsx");
...
it("persists object type ordering in the active space", () => {
  const controllerSource = readSource("app/_components/workspace/space-controller.tsx");
  const dataSource = readSource("app/_components/workspace/use-space-data.ts");
