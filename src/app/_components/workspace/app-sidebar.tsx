...
import { AppShellContent, AppShellHeader } from "@/app/_components/workspace/app-shell";
import {
  AppSidebarAlertIcon,
  AppSidebarCheckIcon,
...
const defaultLabels: AppSidebarLabels = {
  ...
  deleteSpaceDescription: "This removes the local Space from this device. Other Spaces are kept.",
  deleteSpaceError: "Type the exact Space name.",
  nameSpace: "Space name",
  renameSpace: "Rename space",
  saveSpace: "Save changes",
  spaceSettings: "Space settings",
  spaceSettingsDescription: "Rename this Space without affecting content in another Space.",
};
...
function hasActiveSpaceSort(draggingId: string | null, session: DragSession | null) {
...
function AppSidebarSpaceSwitcher({
  ...
  const filteredSpaces = React.useMemo(() => {
    ...
    return displayedSpaces.filter((space) => space.name.toLocaleLowerCase().includes(search));
  ...
  React.useEffect(() => {
    function handlePointerMove(event: PointerEvent) {
      ...
      setDragPreview((preview) =>
        preview ? { ...preview, top: event.clientY - session.offsetY } : preview,
      );
    ...
    function finishPointerSort(event: PointerEvent, commit: boolean) {
      ...
      if (commit && session.moved && !areOrdersEqual(finalOrder, spacesRef.current)) {
          ...
          <AppSidebarSourceIcon
            name={
              (typeof selectedSpace.icon === "string"
                ? selectedSpace.icon
                : "user") as React.ComponentProps<typeof AppSidebarSourceIcon>["name"]
            }
            {...props}
          />
        );
  ...
  function submitDelete(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!selectedSpace) return;
    const deleted = onDeleteSpace?.(selectedSpace.id, deleteConfirmation) ?? false;
    if (!deleted) {
      setDeleteError(true);
      return;
    }
    ...
    return (
      <>
        ...
        <div
          className={cn(
            "min-w-0",
            mobile ? "px-3 pb-[max(0.75rem,env(safe-area-inset-bottom))]" : "px-1.5 pb-1.5",
          )}
        >
          <Button
            type="button"
        ...
        <DialogContent data-slot="app-sidebar-space-create-dialog">
          <form onSubmit={submitCreate} className="contents">
            <DialogHeader>
              ...
              <DialogDescription>{text.spaceSettingsDescription}</DialogDescription>
            </DialogHeader>
            <Input
              autoFocus
        ...
        <DialogContent data-slot="app-sidebar-space-delete-dialog">
          <form onSubmit={submitDelete} className="contents">
            <DialogHeader>
              ...
              <DialogDescription>{text.deleteSpaceDescription}</DialogDescription>
            </DialogHeader>
            <Input
              autoFocus
              aria-label={text.deleteSpaceConfirmation.replace("{name}", selectedSpace.name)}
            ...
            />
            {deleteError && <p className="text-sm text-destructive">{text.deleteSpaceError}</p>}
            <DialogFooter>
