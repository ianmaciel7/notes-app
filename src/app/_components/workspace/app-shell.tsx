...
import { AppHeaderSidebarSimpleIcon } from "@/app/_components/workspace/app-header-icons";
...
function AppShellProvider({ className, children, style, ...props }: React.ComponentProps<"div">) {
  ...
  const [resizingSide, setResizingSide] = React.useState<AppShellResizeSide | null>(null);
  ...
  React.useEffect(() => {
    ...
    const syncLeftWidth = () => {
      root.style.setProperty("--app-shell-left-width", `${panel.getBoundingClientRect().width}px`);
    };
  ...
  return (
    <>
      <ResizablePanel
        ...
        {...props}
      >
        <aside data-slot="app-shell-sidebar" className="flex h-full min-w-0 flex-col">
          {children}
        </aside>
      </ResizablePanel>
  ...
  return (
    <>
      <ResizablePanel
        ...
        {...props}
      >
        <main data-slot="app-shell-main" className="flex h-full min-w-0 flex-col">
          {children}
        </main>
      </ResizablePanel>
  ...
  return (
    <ResizablePanel
      ...
      className={cn(
        ...
        rightCollapsed ? "pointer-events-none opacity-0" : "pointer-events-auto opacity-100",
        className,
      )}
      {...props}
    >
...
function AppShellHeader({ className, style, ...props }: React.ComponentProps<"header">) {
  return (
    <header
...
function AppShellSidePanelTrigger({
  ...
  return (
    <div data-slot="app-shell-side-panel-trigger" className="absolute right-[26px] top-[9px] z-50">
      <Button
...
function AppShellMobileSidebar({
  ...
  const t = useTranslations("workspace.shell");
  const { leftOverlayOpen, setLeftOverlayOpen, leftOverlayReturnFocusRef } = useAppShell();
  return (
...
function AppShellMobileSidePanel({
  ...
  const t = useTranslations("workspace.shell");
  const { rightOverlayOpen, setRightOverlayOpen, rightOverlayReturnFocusRef } = useAppShell();
  return (
    <Sheet open={rightOverlayOpen} onOpenChange={setRightOverlayOpen}>
      <SheetTrigger
        render={
