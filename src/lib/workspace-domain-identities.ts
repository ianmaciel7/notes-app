...
type WorkspaceReverseProjections = {
  readonly collectionMembershipsByCollectionId: ReadonlyMap<
    CollectionId,
    readonly string[]
  >;
...
function migrateLegacyTagNames(
  ...
  return names.flatMap((name) => {
    const normalized = normalizeIdentityName(name);
    return normalized
      ? [{ id: createTagId(normalized, usedIds), name: normalized }]
      : [];
  });
}
...
function migrateLegacyCollectionsByStructure(
  ...
  const usedIds = new Set<string>();
  return Object.entries(collectionsByStructureId).flatMap(
    ([structureId, names]) =>
      names.flatMap((name) => {
        const normalized = normalizeIdentityName(name);
        return normalized
