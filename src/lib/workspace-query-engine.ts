...
function parseWorkspaceSearchQuery(query: string): WorkspaceSearchQueryIntent {
  ...
  if (raw.startsWith("^")) {
    ...
    return {
      ...
      normalized,
      raw,
      terms: splitSearchTerms(normalized),
    };
  }
...
function matchesScalarPropertyOperation(
  actual: unknown,
  expected: unknown,
  operator: QueryPropertyOperator,
): boolean {
...
function relationTargets(
  ...
  return record?.type === "entity"
    ? record.entity.map((target) => target.id)
    : [];
}
...
function contentTargets(
  index: WorkspaceObjectLinkIndex | undefined,
  id: string,
) {
...
function backlinkSources(
  ...
  return (index?.backlinksByTargetId.get(id) ?? []).map(
    (link) => link.sourceId,
  );
}
function matchesReferenceFilter(
  ...
  const references =
    filter.kind === "backlink"
      ? backlinkSources(index, entity.id)
      : contentTargets(index, entity.id);
  const contains = expectedTargetIds(filter.target, variables).some((target) =>
...
function matchesFilter(
  ...
  if (filter.kind === "backlink" || filter.kind === "content-link")
    return matchesReferenceFilter(entity, filter, variables, index);
...
function resolveVariable(
  ...
  entities: ReadonlyMap<string, WorkspaceEntity>,
  ...
  const value = entities.get(context.hostObjectId)
    ? readEntityProperty(
        entities.get(context.hostObjectId) as WorkspaceEntity,
        definition.propertyId,
      )
    : undefined;
  if (Array.isArray(value)) {
    return value.filter(
      (item): item is QueryScalar =>
...
function applySourceFamily(
  ...
  if (query.source === "object-type")
    return entities.filter(
      (entity) => entity.objectTypeId === query.sourceValue,
    );
  if (query.source === "tag")
    return entities.filter(
      (entity) =>
        "tags" in entity && entity.tags.includes(query.sourceValue as string),
    );
  const value = resolveQueryValue(
    query.source === "variable"
      ? { kind: "variable", name: query.sourceValue }
      : query.sourceValue,
    variables,
  );
...
function evaluateQuery(
  ...
  const resolved = resolveVariables(query, activeEntities, context);
  if (resolved.missing.length > 0)
    return { missingVariables: resolved.missing, status: "unresolved" };
  const filtered = applySourceFamily(
    activeEntities,
    query,
    resolved.values,
  ).filter((entity) =>
  ...
  const selected = applySelection(
    sortQueryItems(filtered, query.sorts),
    query.selection,
  );
  const items =
...
function collectBlockEntries(
  ...
  for (const node of nodes) {
    ...
    if (node.content)
      collectBlockEntries(entityId, ownerTitle, node.content, output);
  }
}
...
function buildWorkspaceSearchIndex(
  ...
  const objects = selectActiveEntities(entities).map(
    (entity): SearchIndexObjectEntry => {
      const aliases =
        "aliases" in entity && Array.isArray(entity.aliases)
          ? entity.aliases
          : [];
...
function fieldScore(
  ...
  const queryTerms =
    query.mode === "exact-phrase"
      ? splitSearchTerms(query.normalized)
      : query.terms;
  return scoreTermFieldMatch(valueTerms, queryTerms, query, base);
}
function scoreObjectSearchEntry(
  item: SearchIndexObjectEntry,
  query: WorkspaceSearchQueryIntent,
): number {
  return Math.max(
...
function scoreBlockSearchEntry(
  item: SearchIndexBlockEntry,
  query: WorkspaceSearchQueryIntent,
): number {
...
function validateQueryDefinition(value: unknown) {
  if (!isRecord(value) || value.version !== 1)
    return { error: "QueryDefinition must use version 1.", ok: false } as const;
  if (!sourceKinds.has(value.source as QuerySourceKind))
    return { error: "Query source is invalid.", ok: false } as const;
  if (!resultKinds.has(value.resultKind as QueryResultKind))
    return { error: "Query result kind is invalid.", ok: false } as const;
  if (!isFilterGroup(value.filters) || !hasValidSorts(value.sorts))
    return { error: "Query filters or sorts are invalid.", ok: false } as const;
  if (!hasValidSelection(value.selection) || !isRecord(value.variables))
    return {
      error: "Query selection or variables are invalid.",
      ok: false,
    } as const;
  if (
...
function collectStructureDependencies(
  group: QueryFilterGroup,
  ...
  for (const filter of group.filters) {
    if ("filters" in filter) collectStructureDependencies(filter, structures);
      ...
      for (const id of filter.structureIds) structures.add(id);
  }
}
