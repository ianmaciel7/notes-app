...
export type NumberPresentationColor = "blue" | "gray" | "green" | "orange" | "purple" | "red";
...
type LegacyStructureOverrides = Partial<
  Pick<WorkspaceStructure, "iconName" | "lifecycleKind" | "pluralName" | "singularName" | "tone">
>;
...
function hasUniqueNonEmptyStrings(values: unknown): values is readonly string[] {
  return (
...
function clonePropertyDefinition(definition: PropertyDefinition): PropertyDefinition {
  return {
    ...definition,
    ...(definition.description !== undefined ? { description: definition.description } : {}),
    ...(definition.iconName !== undefined ? { iconName: definition.iconName } : {}),
    ...
    ...(definition.valueType === "number"
      ? {
          numberPresentation: cloneNumberPresentation(definition.numberPresentation),
        }
      : {}),
...
function clonePresentation(presentation: StructurePresentation): StructurePresentation {
  return {
    availableViews: [...presentation.availableViews],
    defaultView: presentation.defaultView,
    ...(presentation.smallCardVisiblePropertyIds
      ? {
          smallCardVisiblePropertyIds: [...presentation.smallCardVisiblePropertyIds],
        }
      : {}),
  };
}
...
function cloneStructure(structure: WorkspaceStructure): WorkspaceStructure {
  return {
    ...
    propertyDefinitions: structure.propertyDefinitions.map(clonePropertyDefinition),
  };
}
...
function mergeDefaultPropertyDefinitions(
  ...
  const explicitIds = new Set(propertyDefinitions.map((definition) => definition.id));
  return [
    ...DEFAULT_PROPERTY_DEFINITIONS.filter((definition) => !explicitIds.has(definition.id)).map(
...
function defineStructure(
  ...
  return deepFreeze({
    ...
    propertyDefinitions: DEFAULT_PROPERTY_DEFINITIONS.map(clonePropertyDefinition),
    singularName,
    tone,
  });
}
...
export const BUILT_IN_STRUCTURES = deepFreeze([
  ...
  defineStructure("tweet", "Tweet", "Tweets", "tweet", "blue", "url", "built-in"),
  defineStructure("ai-chat", "AI chat", "AI chats", "ai-chat", "purple", "document", "built-in"),
  defineStructure("tag", "Tag", "Tags", "tag", "orange", "tag", "built-in"),
  defineStructure("query", "Query", "Queries", "query", "green", "query", "built-in"),
] satisfies readonly WorkspaceStructure[]);

export const RESERVED_STRUCTURES = deepFreeze([
  defineStructure("archive", "Archive", "Archive", "archive", "gray", "document", "reserved"),
] satisfies readonly WorkspaceStructure[]);
...
function validatePresentation(value: unknown): DomainResult<StructurePresentation> {
  if (!isRecord(value)) {
    return failure("invalid-presentation", "Structure presentation must be an object.");
  }
  ...
  return ok({
    ...
    ...(smallCardVisiblePropertyIds === undefined
      ? {}
      : {
          smallCardVisiblePropertyIds: [...(smallCardVisiblePropertyIds as readonly string[])],
        }),
  });
}

function normalizePresentationView(value: unknown): StructurePresentationView | null {
  if (value === "grid") return "gallery";
  if (value === "calendar") return "list";
  return typeof value === "string" && presentationViews.includes(value as StructurePresentationView)
    ? (value as StructurePresentationView)
    : null;
}

function validateLabelOptions(value: unknown): value is readonly PropertyLabelOption[] {
...
function isNumberPresentationColor(value: unknown): value is NumberPresentationColor {
  return (
    typeof value === "string" && numberPresentationColors.includes(value as NumberPresentationColor)
  );
}
...
function readPropertyDefinitionFields(value: Record<string, unknown>): PropertyDefinitionFields {
  return {
...
function hasValidPropertyDefinitionCore(fields: PropertyDefinitionFields): boolean {
  return (
...
function hasValidOptionalIcon(iconName: unknown): boolean {
  return (
    iconName === undefined ||
    (typeof iconName === "string" && OBJECT_ICON_NAMES.includes(iconName as ObjectIconName))
  );
}

function validatePropertyCollectionFields(fields: PropertyDefinitionFields): DomainResult<void> {
...
function validatePropertyReferenceFields(fields: PropertyDefinitionFields): DomainResult<void> {
  if (
  ...
  if (fields.options !== undefined && fields.valueType !== "label") {
    return failure("invalid-property-definition", "Only label properties may define options.");
  }
  if (fields.targetStructureIds !== undefined && fields.valueType !== "entity") {
    return failure(
      "invalid-property-definition",
      "Only entity properties may constrain target Structures.",
    );
  }
  if (fields.fixedTargetObjectIds !== undefined && fields.valueType !== "entity") {
    return failure(
      "invalid-property-definition",
      "Only entity properties may define fixed target objects.",
    );
  }
  if (fields.inversePropertyDefinitionId !== undefined && fields.valueType !== "entity") {
    return failure(
      "invalid-property-definition",
...
function cloneValidatedPropertyDefinition(fields: PropertyDefinitionFields): PropertyDefinition {
  const options = fields.options as readonly PropertyLabelOption[] | undefined;
  const targetStructureIds = fields.targetStructureIds as readonly string[] | undefined;
  const fixedTargetObjectIds = fields.fixedTargetObjectIds as readonly string[] | undefined;
  ...
  return {
    ...
    ...(fixedTargetObjectIds ? { fixedTargetObjectIds: [...fixedTargetObjectIds] } : {}),
    ...(fields.iconName !== undefined ? { iconName: fields.iconName as ObjectIconName } : {}),
    ...(fields.inversePropertyDefinitionId !== undefined
      ? {
          inversePropertyDefinitionId: (fields.inversePropertyDefinitionId as string).trim(),
        }
      : {}),
    ...
    ...(targetStructureIds ? { targetStructureIds: [...targetStructureIds] } : {}),
...
function validateStructureNames(
  ...
  if (!isNonEmptyString(singularName) || !isNonEmptyString(pluralName)) {
    return failure("invalid-name", "Structure singular and plural names must not be empty.");
  }
  return ok(undefined);
}
...
function isSupportedOwnership(value: unknown): value is StructureOwnership {
  return (
    typeof value === "string" &&
    (["built-in", "custom", "legacy", "reserved"] as const).includes(value as StructureOwnership)
  );
}
...
function validateStructureKindAndAppearance(
  ...
  if (!isSupportedOwnership(ownership)) {
    return failure("invalid-structure", "Structure ownership is not supported.");
  }
  ...
  if (
    ...
    return failure("invalid-lifecycle-kind", "Structure lifecycle kind is not supported.");
  }
  return ok(undefined);
}

function validateStructureCollectionIds(collectionIds: unknown): DomainResult<readonly string[]> {
  return hasUniqueNonEmptyStrings(collectionIds)
    ? ok(collectionIds)
    : failure("invalid-structure", "Structure collection ids must be unique non-empty strings.");
}
...
function validateUniqueStructureIds(structures: readonly WorkspaceStructure[]): DomainResult<void> {
  const ids = structures.map((structure) => structure.id);
  return new Set(ids).size === ids.length
    ? ok(undefined)
    : failure("duplicate-id", "Structure ids must be unique within the registry.");
}
...
function findStructureDefinition(
  ...
  return (
    structures
      ...
      ?.propertyDefinitions.find((definition) => definition.id === propertyDefinitionId) ?? null
  );
}
...
function isCompatibleInversePair(
  ...
  if (definition.fixedTargetObjectIds?.length || inverse?.fixedTargetObjectIds?.length) {
    return false;
  }
  return (
...
function validateInverseTargets(
  ...
  for (const targetStructureId of definition.targetStructureIds ?? []) {
    if (!isCompatibleInversePair(structures, structure.id, targetStructureId, definition)) {
      return failure(
        "invalid-property-definition",
...
export function validatePropertyDefinition(value: unknown): DomainResult<PropertyDefinition> {
  if (!isRecord(value)) {
    return failure("invalid-property-definition", "Property definition must be an object.");
  }
  const fields = readPropertyDefinitionFields(value);
  if (!hasValidPropertyDefinitionCore(fields) || !hasValidOptionalIcon(fields.iconName)) {
    return failure(
      "invalid-property-definition",
...
export function validateWorkspaceStructure(value: unknown): DomainResult<WorkspaceStructure> {
  if (!isRecord(value)) return failure("invalid-structure", "Structure must be an object.");
  const {
    collectionIds,
    iconName,
    id,
...
function replaceAt(
  ...
  return registry.map((current, currentIndex) => (currentIndex === index ? structure : current));
}

function ensureEditable(structure: WorkspaceStructure): DomainResult<WorkspaceStructure> {
...
export function createCustomStructure(
  ...
  if (registry.some((structure) => structure.id === id)) {
    return failure("id-collision", `Generated Structure id already exists: ${id}`, { id });
  }
  const candidate: WorkspaceStructure = {
    ...
    propertyDefinitions: mergeDefaultPropertyDefinitions(input.propertyDefinitions),
...
function propertyDefinitionChanged(before: PropertyDefinition, after: PropertyDefinition): boolean {
...
export function replaceStructureSchema(
  ...
  const nextById = new Map(
    validation.value.propertyDefinitions.map((definition) => [definition.id, definition]),
  );
  const unsafeMutation = editable.value.propertyDefinitions.find((definition) => {
...
export function deleteStructure(
  ...
  if (!Number.isSafeInteger(instanceCount) || instanceCount < 0) {
    return failure("structure-in-use", "Structure instance count must be a non-negative integer.");
  }
  if (instanceCount > 0) {
    return failure("structure-in-use", `Structure ${id} still has object instances.`, {
      id,
      instanceCount,
    });
  }
  ...
  if (dependentCollectionIds.length > 0) {
    return failure("dependent-collections", `Structure ${id} still has dependent collections.`, {
...
export function instantiateObjectTypePreset(
  ...
  const preset = OBJECT_TYPE_PRESETS.find((candidate) => candidate.id === presetId);
  if (!preset)
    return failure("unknown-preset", `Unknown object type preset: ${presetId}`, { presetId });
  return createCustomStructure(
    registry,
    {
      ...
      propertyDefinitions: preset.propertyDefinitions.map(clonePropertyDefinition),
...
export function createLegacyStructureDefinition(
  ...
  if (!preset) {
    return failure("unknown-structure", `No legacy Structure definition exists for ${id}.`, { id });
  }
  const candidate: WorkspaceStructure = {
    collectionIds: [],
...
export function createInitialStructureRegistry(): readonly WorkspaceStructure[] {
  return [...BUILT_IN_STRUCTURES.map(cloneStructure), ...RESERVED_STRUCTURES.map(cloneStructure)];
}
...
export function selectCustomStructures(
  ...
  return registry.filter(
    (structure) => structure.ownership === "custom" || structure.ownership === "legacy",
  );
}
...
export function selectCreatableStructures(
  ...
  return registry.filter(
    (structure) => structure.ownership === "built-in" || structure.ownership === "custom",
  );
}
