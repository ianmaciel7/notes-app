...
export type StructureDashboardSection = {
  ...
  readonly id: string;
  readonly kind: "data-view" | "recent" | "template-shortcuts";
  readonly limit?: number;
...
function isDashboard(value: unknown): value is StructureDashboard {
  ...
  return (
    ...
    dashboard.sections.every(
      (section) =>
        isNonEmptyString(section.id) &&
        isNonEmptyString(section.title) &&
        (section.kind === "data-view" ||
          section.kind === "recent" ||
          section.kind === "template-shortcuts") &&
        (section.kind !== "data-view" || isNonEmptyString(section.dataViewId)),
    )
  );
}
...
function hasValidSnapshotRecords(snapshot: WorkspaceViewState): boolean {
  return (
    snapshot.dashboards.every(isDashboard) &&
...
export function parseWorkspaceViewState(
  ...
  return dataViews.ok ? success(snapshot) : dataViews;
}
