...
describe("Database Repositories", () => {
  ...
  describe("SpaceRepository", () => {
    ...
    it("createSpace validates name and generates unique ID", async () => {
      ...
      const space2 = await repos.spaces.createSpace({
        name: "Project B",
      });
    ...
    it("deleteSpace cascades deletion across all tables and returns fallback space", async () => {
      ...
      // Seed records across tables for the space to be deleted
      ...
      // Seed an entity in Personal space to verify it is NOT touched
      await db.entities.put({
      ...
      // Verify cascade deletion for target space
      ...
      // Verify Personal space was preserved
      expect(await db.entities.get([PERSONAL_SPACE_ID, "personal-ent"])).toBeDefined();

      // Verify active space setting updated to fallback
      const activeSetting = await db.appSettings.get(ACTIVE_SPACE_SETTING_ID);
      expect(activeSetting?.value).toBe(PERSONAL_SPACE_ID);

      // Verify cannot delete only remaining space
  ...
  describe("RelationRepository", () => {
    ...
    it("listOutgoingRelations and listIncomingBacklinks return expected records", async () => {
      ...
      const rel1 = await repos.relations.createRelation(
        spaceId,
        source.id,
        target1.id,
        "specifies"
      );
      const rel2 = await repos.relations.createRelation(
        spaceId,
        source.id,
        target2.id,
        "specifies"
      ...
      // Verify source entity relations array was updated
  ...
  describe("TrashRepository (Graph-Safe Soft-Deletion)", () => {
    it("moveToTrash moves entity to trash table, deletes incident relations, and captures snapshots", async () => {
      ...
      // upstream -> center (incoming to center)
      ...
      // center -> downstream (outgoing from center)
      ...
      // Verify record exists in trash table
  ...
  describe("SyncMutationRepository", () => {
    it("enqueueMutation performs LWW coalescing for pending writes on same entity", async () => {
      ...
      // 2. Enqueue second mutation on the same entity while still pending -> coalesces
      ...
      // 3. Mark as syncing and coalesce another update
      ...
      // 4. Mark as synced. Next mutation should NOT coalesce into synced mutation.
