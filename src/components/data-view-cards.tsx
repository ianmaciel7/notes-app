...
function dataViewCardsClass(view: WorkspaceDataView): string {
  ...
  if (presentation.kind === "gallery") {
    if (presentation.cardSize === "small") {
      return "grid grid-cols-[repeat(auto-fill,minmax(10rem,1fr))] gap-3";
    }
    if (presentation.cardSize === "large") {
      return "grid grid-cols-[repeat(auto-fill,minmax(20rem,1fr))] gap-3";
    }
  }
  return "grid grid-cols-[repeat(auto-fill,minmax(14rem,1fr))] gap-3";
}
