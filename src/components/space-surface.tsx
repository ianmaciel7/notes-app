...
const workspaceRouteClass = "relative flex h-full min-h-0 flex-col bg-card text-foreground";
...
const workspaceSectionTitleClass = "text-sm font-medium leading-5 text-foreground";

const workspaceMetaTextClass = "text-[13px] leading-5 text-muted-foreground";

const workspaceReferenceSecondaryTextClass = "text-[var(--app-text-secondary)]";

const workspaceCardGridClass = "grid w-full grid-cols-1 gap-2 sm:grid-cols-2";

const workspaceListSurfaceClass = "overflow-hidden rounded-[12px] border border-border bg-card p-1";
...
function WorkspaceEmptyState({
  ...
  return (
    ...
    >
      ...
      {description ? (
        <p className={cn(workspaceMetaTextClass, "mt-1 max-w-md")}>{description}</p>
      ) : null}
