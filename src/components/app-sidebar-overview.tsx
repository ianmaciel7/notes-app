...
const objectTypeMenuIconPaths = {
  ...
  present:
    "M216 40h-80V24a8 8 0 0 0-16 0v16H40a16 16 0 0 0-16 16v120a16 16 0 0 0 16 16h39.36l-21.61 27a8 8 0 0 0 12.5 10l29.59-37h56.32l29.59 37a8 8 0 0 0 12.5-10l-21.61-27H216a16 16 0 0 0 16-16V56a16 16 0 0 0-16-16m0 136H40V56h176z",
  settings:
...
function AppSidebarObjectTypeMenuIcon({
  ...
  return (
    <span {...props} className={cn("flex size-3 items-center justify-center", className)}>
    ...
    <DropdownMenu open={open} onOpenChange={setOpen}>
      ...
      >
        ...
        >
          ...
          <CompactMenuItemText>{t("manual")}</CompactMenuItemText>
          {value === "manual" && <AppSidebarCheckIcon className="ml-auto" />}
        </DropdownMenuItem>
        <DropdownMenuItem
          className={sidebarContextMenuItemClass}
          onClick={() => onValueChange("alphabetical")}
        >
          <SidebarContextMenuIcon>
            <AppSidebarArrowDownIcon />
          </SidebarContextMenuIcon>
          <CompactMenuItemText>{t("alphabetical")}</CompactMenuItemText>
            ...
            <div className="grid gap-3 py-4">
              ...
              <label htmlFor={settingsIconInputId} className="grid gap-1 text-sm">
                ...
                <select
                  ...
                  value={iconName}
                  onChange={(event) => setIconName(event.target.value as ObjectIconName)}
              ...
              <label htmlFor={settingsToneInputId} className="grid gap-1 text-sm">
                ...
                <select
                  ...
                  value={tone}
                  onChange={(event) => setTone(event.target.value as AppSidebarTone)}
                  className="h-9 rounded-md border bg-background px-2"
                >
    ...
    <div data-slot="app-sidebar-object-type-row-wrapper" className="mx-2">
      ...
      >
        {hasCollections && (
          <button
            type="button"
            aria-label={t(collectionsOpen ? "hideCollections" : "showCollections", {
      ...
      <Popover open={open} onOpenChange={setOpen}>
        ...
        >
          <PopoverHeader>
            ...
            <PopoverDescription>{t("sidebarSections.description")}</PopoverDescription>
          </PopoverHeader>
...
function AppSidebarTrashRow({
  ...
  const visibleItems = React.useMemo(() => {
    ...
    return normalized
      ? items.filter((item) =>
          [item.label, item.typeLabel].join(" ").toLocaleLowerCase().includes(normalized),
        )
      : items;
  }, [items, query]);
  ...
  function formatDate(value: string) {
    ...
    return Number.isFinite(date.valueOf()) ? dateFormatter.format(date) : value.slice(0, 10);
  }
      ...
      >
        ...
        >
          <span className="flex w-full min-w-0 items-center">
            ...
            <span className="min-w-0 truncate">{t("sidebarUtilities.trash")}</span>
          </span>
        </PopoverTrigger>
        ...
        >
          <PopoverHeader>
            ...
            <PopoverDescription>{t("sidebarUtilities.trashDescription")}</PopoverDescription>
          </PopoverHeader>
          <Input
            className="h-8"
            value={query}
...
function AppSidebarCollectionMenu({
  ...
  const t = useTranslations("workspace");
  const action = (name: AppSidebarCollectionAction) => () => onAction(name, objectType, collection);
