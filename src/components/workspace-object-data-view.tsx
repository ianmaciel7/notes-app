...
function ObjectTypeDataViewCard({
  ...
  return (
    <li
      data-slot="workspace-object-type-list-item"
      className={cn(layout === "cards" && "min-w-0")}
    >
      <button
        type="button"
      ...
      >
        ...
        {layout === "cards" ? (
          <>
            <div className="flex min-h-8 w-full grow px-2.5">
              <p className="w-full px-0.5 pt-0.5 text-base font-semibold leading-snug !text-[var(--app-text-primary)] line-clamp-2">
                {entity.title || "Sem título"}
              </p>
            </div>
            <div className="h-48 px-2.5">
              ...
              >
                {previewLines.map((line, index) => (
                  <p key={`${entity.id}-${index}`} className="mb-3 line-clamp-3 last:mb-0">
                    {line}
                  </p>
                ))}
              </div>
            </div>
            <div className="flex h-6 w-full items-center overflow-hidden px-2.5 text-xs text-[var(--app-text-secondary)]">
  ...
  function renderItems(groupEntities: readonly SpaceEntityRecord[], groupName: string | null) {
    return (
      <div key={groupName ?? "all"} className={groupName ? "mb-5 last:mb-0" : undefined}>
        ...
        <ul
          data-slot={useCards ? "workspace-object-data-view-cards" : "workspace-object-data-view-list"}
          className={cn(
            useCards
              ? "grid w-full grid-cols-[repeat(auto-fill,minmax(min(100%,12.875rem),12.875rem))] items-start gap-3"
