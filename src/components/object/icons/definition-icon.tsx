import * as React from "react";
import type { ObjectIconProps } from "@/lib/object";
import { ObjectIcon } from "./object-icon";

export const DefinitionIcon = React.forwardRef<SVGSVGElement, ObjectIconProps>(
  function DefinitionIcon(props, ref) {
    return (
      <ObjectIcon ref={ref} data-icon="definition" {...props}>
        <path
          fill="currentColor"
          d="M184,32H72A16,16,0,0,0,56,48V224a8,8,0,0,0,12.24,6.78L128,193.43l59.77,37.35A8,8,0,0,0,200,224V48A16,16,0,0,0,184,32Zm0,177.57-51.77-32.35a8,8,0,0,0-8.48,0L72,209.57V48H184Z"
        />
      </ObjectIcon>
    );
  },
);

DefinitionIcon.displayName = "DefinitionIcon";
export const ObjectDefinitionIcon = DefinitionIcon;
