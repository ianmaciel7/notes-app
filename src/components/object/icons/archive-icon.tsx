import * as React from "react";
import type { ObjectIconProps } from "@/lib/space-object-types";
import { ObjectIcon } from "./object-icon";

export const ArchiveIcon = React.forwardRef<SVGSVGElement, ObjectIconProps>(
  function ArchiveIcon(props, ref) {
    return (
      <ObjectIcon ref={ref} data-icon="archive" {...props}>
        <path
          fill="currentColor"
          d="M224,48H32A16,16,0,0,0,16,64V88a16,16,0,0,0,16,16v88a16,16,0,0,0,16,16H208a16,16,0,0,0,16-16V104a16,16,0,0,0,16-16V64A16,16,0,0,0,224,48ZM208,192H48V104H208ZM224,88H32V64H224V88ZM96,136a8,8,0,0,1,8-8h48a8,8,0,0,1,0,16H104A8,8,0,0,1,96,136Z"
        />
      </ObjectIcon>
    );
  },
);

ArchiveIcon.displayName = "ArchiveIcon";
export const ObjectArchiveIcon = ArchiveIcon;
