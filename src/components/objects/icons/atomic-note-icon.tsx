import * as React from "react";
import type { ObjectIconProps } from "../../../lib/space-object-types";
import { ObjectIcon } from "./object-icon";

export const AtomicNoteIcon = React.forwardRef<SVGSVGElement, ObjectIconProps>(
  function AtomicNoteIcon(props, ref) {
    return (
      <ObjectIcon ref={ref} data-icon="atomic-note" {...props}>
        <path
          fill="currentColor"
          d="M208,88H48a16,16,0,0,0-16,16v96a16,16,0,0,0,16,16H208a16,16,0,0,0,16-16V104A16,16,0,0,0,208,88Zm0,112H48V104H208v96ZM48,64a8,8,0,0,1,8-8H200a8,8,0,0,1,0,16H56A8,8,0,0,1,48,64ZM64,32a8,8,0,0,1,8-8H184a8,8,0,0,1,0,16H72A8,8,0,0,1,64,32Z"
        />
      </ObjectIcon>
    );
  },
);

AtomicNoteIcon.displayName = "AtomicNoteIcon";
export const ObjectAtomicNoteIcon = AtomicNoteIcon;
