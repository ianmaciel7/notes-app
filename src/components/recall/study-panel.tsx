            ...
            onSubmit={async (event) => {
              ...
              try {
                const value = await startSession(
                  ...
                  String(form.get("mode")) as StudySession["mode"],
                );
                setSession(value);
              ...
              <Field>
                ...
                >
                  ...
                  <option value="simulated_exam">
                    Simulated exam · 90 seconds per question
                  </option>
                </select>
              </Field>
              {error && (
          ...
          {session.results.map((result) => (
            <div key={result.objectId} className="rounded-lg border p-4">
              <p className="font-medium">
                {result.correct ? "✓" : "↻"}{" "}
                {
                  session.questions.find((item) => item.id === result.objectId)
                    ?.title
                }
              </p>
    ...
    <Card>
      ...
      <CardContent className="flex flex-col gap-5">
        ...
        <p className="text-xs text-muted-foreground">
          Answers are saved when you continue. You can resume this session after
          a reload.
        </p>
      </CardContent>
    </Card>
