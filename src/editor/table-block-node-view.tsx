...
function selectedCellIds(table: TableBlockModel, selection: TableSelection) {
  ...
  const rowStart = Math.min(
    rowIds.indexOf(anchor.rowId),
    rowIds.indexOf(focus.rowId),
  );
  ...
  return table.rows.slice(rowStart, rowEnd + 1).flatMap((row) =>
    table.columns
...
function nextCell(table: TableBlockModel, cell: TableBlockCell, key: string) {
  ...
  return table.cells[
    cellKey(
      ...
      table.columns[Math.min(Math.max(nextColumn, 0), table.columns.length - 1)]
        .id,
    )
  ];
}
...
function TableBlockNodeView({ editor, node, updateAttributes }: NodeViewProps) {
  ...
  const applyCommand = React.useCallback(
    (command: TableBlockCommand) => {
      ...
      updateAttributes({
        ...
        table: result.value.after,
      });
      window.setTimeout(() => {
        editorDom.dispatchEvent(
          new CustomEvent("table-block-change", { bubbles: true }),
        );
      }, 0);
      return true;
    },
    [editable, editorDom, updateAttributes],
  );
  ...
  const activeCellId =
    selection?.focusCellId ?? Object.values(table.cells)[0]?.id;
