...
| Add object type modal | `src/components/app-sidebar-object-type-studio.tsx`, `src/components/object-icons.tsx`, `src/lib/space-object-types.ts` | Modal matches Capacities card grid: explicit light card surface `oklch(1 0.0001 263.28)`, border `oklch(0.9163 0.0017 67.07)`, hover `oklch(0.9856 0.0016 67)`, label text `oklch(0.3887 0.0052 301.05)`, `h-base w-base` icon badge, and `text-sm font-semibold` labels. | Current screenshot tone map is enforced by shared `getCapacitiesObjectTypeTone`: colored `Livro`, `Área`, `Definição`, `Ideia`, `Projeto`, `Mídia`, `Viagem`, `Página`, `Weblink`, `Tweet`, `Chat de IA`, `Tabela`, `Query`; gray `Pessoa`, `Reunião`, `Citação`, `Lugar`, `Organização`, `Nota atômica`, `Crie o seu próprio`, `Etiqueta`, `Imagem`, `PDF`, `Áudio`, `Arquivo`, `Tarefa`. |
