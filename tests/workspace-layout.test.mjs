...
test("workspace panel presentation preserves the desktop sidebar breakpoint", () => {
  assert.equal(WORKSPACE_MOBILE_BREAKPOINT, 769);
  ...
  assert.equal(getWorkspacePanelPresentation(768), "mobile");
  assert.equal(getWorkspacePanelPresentation(769), "overlay");
  assert.equal(getWorkspacePanelPresentation(1023), "overlay");
  assert.equal(getWorkspacePanelPresentation(1024), "overlay");
  assert.equal(getWorkspacePanelPresentation(1025), "inline");
...
test("workspace layout exports the current reference shell geometry", () => {
  ...
  assert.equal(WORKSPACE_CONTEXT_PANEL_DEFAULT_SIZE_PERCENT, 35.5);
  assert.equal(WORKSPACE_CONTEXT_PANEL_MIN_WIDTH_PX, 240);
