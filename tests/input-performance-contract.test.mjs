...
test("workspace editors use buffered commits for text-heavy object fields", async () => {
  const [workspaceContent, blockEditor, documentCommitHook] = await Promise.all(
    [
      readFile(
      ...
      readFile(
        new URL(
          "../src/editor/use-buffered-document-commit.ts",
          import.meta.url,
        ),
        "utf8",
      ),
  ...
  assert.match(
    workspaceContent,
    /function EditableTitle[\s\S]*useBufferedTextCommit/,
  );
  assert.doesNotMatch(
    workspaceContent,
  ...
  assert.match(documentCommitHook, /pagehide/);
  assert.match(documentCommitHook, /beforeunload/);
  assert.match(documentCommitHook, /flushPendingDocument/);
});
