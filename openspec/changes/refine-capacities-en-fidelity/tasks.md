...
- [x] 1.1 Re-audit Git status, preserve unrelated `graphify-out/*` and `.playwright-mcp/` changes, and record the exact available branch/reference corpus state.
- [x] 1.2 Consolidate the component audit and 16-category mismatch ledger from live target, current source, Graphify, and selective donor evidence.
- [x] 1.3 Capture equal-viewport baseline screenshots/computed geometry for the accepted target and local `/en` states at the supported desktop checkpoints.

## 2. Acceptance Composition

- [x] 2.1 Read the installed Next.js 16 font guidance before any Next-specific font change and implement the confirmed shared typography/surface foundation without broad component-specific global CSS.
- [x] 2.2 Add a route-owned, typed Capacities fidelity fixture/component for the `Notas atômicas` main surface and `Explorar` contextual content using existing shadcn primitives and stable `data-slot` contracts.
- [x] 2.3 Compose the fixture into `src/app/[locale]/page.tsx` and align the controlled header/sidebar demo state with the exact accepted target without adding persistence or routing behavior.

## 3. Evidence-Backed Component Refinement
