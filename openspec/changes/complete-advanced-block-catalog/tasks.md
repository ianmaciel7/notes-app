...
- [x] 2.1 Implement toggle and emoji text interfaces with keyboard, selection, read-only, and persistence tests.
- [x] 2.2 Implement highlight blocks with source metadata and safe import/render behavior.
- [x] 2.3 Implement language-aware code blocks, copy/download actions, Mermaid rendering, and invalid-diagram fallback.
- [x] 2.4 Implement TeX math blocks and inline math with source-preserving error states.

## 3. Layout and Group Blocks

- [x] 3.1 Implement group/ungroup transactions preserving child IDs, order, selection, and undo/redo.
- [x] 3.2 Implement multi-column/grid layout creation, reorder, resize policy, responsive linearization, and accessible reading order.
- [x] 3.3 Add width and appearance controls through shared editor primitives.

## 4. Object Blocks

- [x] 4.1 Complete inline, small-card, wide-card, embed/transclusion, and supported media display variants.
- [x] 4.2 Add recursion, missing-target, permission, offline, and read-only fallbacks.
