...
- Side-panel special entries follow the Capacities tab model: when the default `Explore` tab is the only side tab, choosing Graph view, Backlinks, Objects inside, Related content, AI chat, or Search replaces that single `Explore` tab with the chosen item. When two or more side tabs are already open, choosing an item preserves the existing tabs and selects or appends the chosen item like the main workspace tabs.
