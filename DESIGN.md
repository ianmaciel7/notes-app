...
- Sidebar object-type/collection, pinned-object, and section action menus mirror the Capacities popup root captured via `document.querySelector("#popup-container > div")`: use the shared `sidebarContextMenu*` helpers from `src/components/ui/compact-menu.tsx`, with `preview-card-core`, `bg-front`, `border-front`, `text-subtle`, `text-left text-xs font-sans font-normal`, `shadow-[var(--app-shadow-sidebar-popover)]`, a visible popup width around `269px` implemented as `box-content w-[257px]` plus `p-1.5`, `h-base`/`32px` menu rows, `pl-1 pr-1` row padding, ghost icon frames with a `12px` inner icon, `bg-el` for hover/highlight/open submenu state, and `0.5px` `border-front` separators between action groups. Destructive object actions keep the row text `text-primary`; only the left trash icon frame becomes `text-red-500 dark:text-red-400`.
...
- Deep workspace URLs must hydrate from a stable default shell first, then restore the active route after mount. Do not read `window.location` or `localStorage` while constructing initial React state for SSR-rendered workspace tabs.
