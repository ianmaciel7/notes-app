...
- Sidebar object-type/collection action menus mirror the Capacities root captured via `document.querySelector("#\\39 fb64eba-1331-485b-b2a2-5c329d759d65")`: use a `269px` popup width, `p-1.5` container padding, `h-base`/`32px` menu rows, `pl-1 pr-1` row padding, ghost icon frames with a `12px` inner icon, `bg-el` for hover/highlight state, and `0.5px` `border-front` separators between action groups.
