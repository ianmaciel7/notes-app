...
## Corpus Check
- 340 files · ~516,074 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 4339 nodes · 7630 edges · 322 communities (218 shown, 76 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 132 edges (avg confidence: 0.86)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `a995a6e1`
...
## Communities (322 total, 76 thin omitted)

### Community 0 - "_make_id"
Cohesion: 0.02
Nodes (147): _import_csharp(), _import_js(), _import_kotlin(), extract_apex(), Path, Apex extractor. Moved verbatim from graphify/extract.py., Extract classes, interfaces, enums, methods, and Salesforce constructs from…, _file_stem() (+139 more)

### Community 1 - "_read_text"
Cohesion: 0.02
Nodes (178): _get_c_func_name(), _import_java(), _import_php(), _import_scala(), _import_swift(), Emit module-level ``imports`` edges and report the imported modules. A Swift…, Recursively unwrap declarator to find the innermost identifier (C)., _read_text() (+170 more)
...
### Community 3 - "cli.py"
Cohesion: 0.04
Nodes (86): distinct_repo_tags(), edge_datas(), prefix_graph_for_global(), prune_repo_from_graph(), Graph, Return a copy of G with all node IDs prefixed with repo_tag::. Labels are…, Return a unique, human-meaningful repo tag per input graph for merge-graphs.…, Remove all nodes tagged with repo_tag from G in-place. Returns count removed. (+78 more)

### Community 4 - "extract"
Cohesion: 0.06
Nodes (41): _canonicalize_csharp_namespace_nodes(), _check_tree_sitter_version(), extract(), _file_node_id(), _is_top_level_function_definition(), _lang_family(), _lang_is_case_insensitive(), _merge_csharp_partial_class_nodes() (+33 more)

### Community 5 - "extract.py"
Cohesion: 0.04
Nodes (136): _augment_js_reexport_edges(), _emit_rescued_import(), extract_astro(), extract_svelte(), extract_vue(), _import_c(), _import_lua(), _import_python() (+128 more)

### Community 6 - "Path"
Cohesion: 0.03
Nodes (91): _augment_cpp_string_tests(), _blank_keeping_newlines(), extract_c(), extract_cpp(), extract_csharp(), extract_groovy(), extract_java(), extract_js() (+83 more)
...
Cohesion: 0.08
Nodes (53): _bfs(), _community_header(), _complete_induced_edges(), _compute_idf(), _cut_lines_to_budget(), _dfs(), _display_graph_path(), _filter_graph_by_context() (+45 more)
...
### Community 20 - "llm.py"
Cohesion: 0.08
Nodes (31): _claude_cli_available(), _community_label_lines(), _custom_providers_path(), generate_community_labels(), _get_tokenizer(), _label_batch_with_retry(), label_communities(), _load_custom_providers() (+23 more)
...
### Community 24 - "Design System: Notes App"
Cohesion: 0.12
...
Cohesion: 0.06
Nodes (31): dom, dom.iterable, esnext, **/*.mts, .next/dev/types/**/*.ts, .next/types/**/*.ts, **/*.stories.tsx, **/*.test.ts (+23 more)
...
### Community 46 - "FileSlice"
Cohesion: 0.11
Nodes (30): extract_pdf_text(), Extract plain text from a PDF file using pypdf., _best_cut(), bisect_slice(), expand_oversized_files(), FileSlice, is_splittable_text(), _pdf_text() (+22 more)
...
### Community 67 - "deduplicate_entities"
Cohesion: 0.16
Nodes (18): _crossfile_fileanchored_blocked(), deduplicate_entities(), _entropy(), _is_variant_pair(), _llm_tiebreak(), _norm(), _numeric_tokens_differ(), _pick_winner() (+10 more)
...
### Community 73 - "html.py"
Cohesion: 0.13
Nodes (19): Shared constants/helpers for the graphify exporters package. Symbols used by…, _html_document_title(), _html_script(), _html_styles(), _hyperedge_script(), Graph, html — moved verbatim from graphify/export.py., Return the effective viz node limit, honoring GRAPHIFY_VIZ_NODE_LIMIT env var.… (+11 more)
...
Cohesion: 0.14
Nodes (14): _content_richness(), _is_code(), _make_minhash(), _merge_missing_attributes(), Entity deduplication pipeline for graphify knowledge graphs. Pipeline: exact…, True for AST-extracted code symbols. Code-node identity is the node ID (which…, Return k-gram character shingles of text., True when exact-ID records came from the same source file. Exact IDs can also… (+6 more)
...
### Community 115 - "_extract_with_adaptive_retry"
Cohesion: 0.17
...
Cohesion: 0.18
Nodes (9): Architectural Decision Log (MADR), Index of Architectural Decision Records (ADRs), AI & Development Setup, Community, Contributing, Documentation, Getting Started, Good First Issues: (+1 more)
...
### Community 186 - "_content_token_swap"
Cohesion: 0.50
...
Cohesion: 0.06
Nodes (65): _abs_identity(), build(), build_from_json(), build_merge(), _build_prune_sets(), _coerce_hyperedge_member_refs(), _coerce_id(), _coerce_non_string_ids() (+57 more)
...
### Community 246 - "Codebase Conventions & Guidelines"
Cohesion: 0.50
Nodes (3): 1. Directory Conventions, 2. Coding Rules & Principles, Codebase Conventions & Guidelines

### Community 248 - "doc-viewer.tsx"
Cohesion: 0.08
Nodes (12): WorkspaceArchitectureLocale, WorkspaceArchitectureStoryProps, workspaceMainContentDocs, ArchitectureDocViewer, detectDarkModeFromDom(), DocBlock, DocViewer(), MermaidDiagram() (+4 more)
...
### Community 323 - "_get_backend_api_key"
Cohesion: 0.25
...
### Community 326 - "_kotlin_package_index"
Cohesion: 0.33
...
### Community 333 - "Main Content Architecture Overview"
Cohesion: 0.50
Nodes (3): Key Features, Main Content Architecture Overview, Render Flow

### Community 334 - "Visão Geral da Arquitetura do Conteúdo Principal"
Cohesion: 0.50
Nodes (3): Fluxo de Renderização, Recursos Principais, Visão Geral da Arquitetura do Conteúdo Principal

## Knowledge Gaps
- **925 isolated node(s):** `$schema`, `enabled`, `clientKind`, `useIgnoreFile`, `ignoreUnknown` (+920 more)
...
- **Why does `dispatch_command()` connect `cli.py` to `detect`, `__main__.py`, `extract`, `tree_html.py`, `cache.py`, `reflect.py`, `serve.py`, `export.py`, `watch.py`, `hooks.py`, `llm.py`, `write_callflow_html`, `_call_llm`, `analyze.py`, `prs.py`, `FileSlice`, `diagnostics.py`, `ingest.py`, `save_manifest`, `affected.py`, `_get_backend_api_key`, `link_cross_repo_member_calls`, `html.py`, `semantic_cleanup.py`, `edge_data`, `_build_server`, `cluster.py`, `introspect_cargo`, `build.py`?**
  _High betweenness centrality (0.050) - this node is a cross-community bridge._
- **Why does `save_semantic_cache()` connect `cache.py` to `cli.py`, `llm.py`, `FileSlice`?**
  _High betweenness centrality (0.026) - this node is a cross-community bridge._
- **Why does `extract()` connect `extract` to `_make_id`, `cli.py`, `extract.py`, `Path`, `cache.py`, `CsharpNameResolver`, `watch.py`, `symbol_resolution.py`, `resolver_registry.py`?**
...
- **Should `_make_id` be split into smaller, more focused modules?**
  _Cohesion score 0.023051815585475317 - nodes in this community are weakly interconnected._
- **Should `_read_text` be split into smaller, more focused modules?**
  _Cohesion score 0.017387483522691607 - nodes in this community are weakly interconnected._