...
export async function gradeScenario({
  ...
  checks.push({
    ...
    detail: unexpected.join(", ") || changed.join(", ") || "no changes",
  });
  ...
  for (const needle of scenario.traceIncludes ?? []) {
    checks.push({
      name: `trace:${needle}`,
      pass: traceText.includes(needle.toLowerCase()),
      detail: needle,
    });
  }
  ...
  for (const expectation of scenario.fileContains ?? []) {
    let content = "";
    try {
      content = await readFile(path.join(workspace, expectation.path), "utf8");
    } catch {}
    checks.push({
      name: `file:${expectation.path}`,
      pass: content.includes(expectation.text),
