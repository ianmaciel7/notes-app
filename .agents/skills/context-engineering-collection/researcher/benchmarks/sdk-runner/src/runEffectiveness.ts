...
async function main(): Promise<number> {
  ...
  if (tasks.length === 0) {
    console.error(
      "No tasks present yet. Add at least one task under researcher/benchmarks/effectiveness/tasks/.",
    );
    return 1;
  }
  ...
  if (config.dryRun) {
    ...
    for (const task of tasks.slice(0, 3)) {
      console.log(
        `  - ${task.id} target=${task.target_skill} difficulty=${task.difficulty}`,
      );
    }
    return 0;
  }
  ...
  mkdirSync(join(RESULTS_DIR, `${todayUtc()}-${config.seed}`), {
    recursive: true,
  });
  return 0;
}
...
function discoverTasks(): EffectivenessTaskMetadata[] {
  ...
  for (const entry of readdirSync(TASKS_DIR).sort()) {
    ...
    try {
      ...
      ) as EffectivenessTaskMetadata;
      tasks.push(meta);
    } catch (error) {
      console.warn(
        `Skipping ${entry}: invalid metadata.json (${(error as Error).message})`,
      );
    }
  }
  return tasks;
