...
export function resolveConfig(
  ...
  return {
    ...
    concurrency:
      flags.concurrency && flags.concurrency > 0 ? flags.concurrency : 1,
...
export function loadJsonl<T>(path: string): T[] {
  ...
  for (let lineIndex = 0; lineIndex < lines.length; lineIndex++) {
    ...
    try {
      ...
      throw new Error(
        `Invalid JSONL at ${path}:${lineIndex + 1}: ${(error as Error).message}`,
      );
    }
  }
  return records;
...
export function loadSkillDescriptions(): Array<{
  name: string;
  description: string;
}> {
...
export function assertBudget(
  ...
  if (plan.length > config.maxRuns) {
    throw new Error(
      `Plan size ${plan.length} exceeds --max-runs ${config.maxRuns}`,
    );
  }
