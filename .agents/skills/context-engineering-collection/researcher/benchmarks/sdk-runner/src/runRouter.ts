...
const ROUTING_PROMPT_TEMPLATE = join(
  RESEARCHER_DIR,
  "benchmarks",
  "router",
  "routing-prompt.md",
...
async function main(): Promise<number> {
  ...
  console.log(
    `est. tokens per run: ${ESTIMATED_TOKENS_INPUT}in / ${ESTIMATED_TOKENS_OUTPUT}out`,
  );
  ...
  const existingResults = config.noResume
    ? new Map<string, RouterRunRecord>()
    : loadExistingResults(runDir);
  const remaining = plan.filter(
    (item) =>
      !existingResults.has(
        resultFileName(item.promptId, item.modelId, item.rep),
      ),
  );
  if (existingResults.size) {
    console.log(
      `resume: ${existingResults.size} prior results found, ${remaining.length} runs remaining`,
    );
  }
  ...
  await runConcurrently(remaining, config.concurrency, async (item, _index) => {
    ...
    try {
      for (let attempt = 1; attempt <= MAX_FORMAT_ATTEMPTS; attempt++) {
        ...
        if (parsed) {
          ...
          record.top3_correct = parsed
            .slice(0, 3)
            .includes(prompt.expected_primary_skill);
          break;
        }
        record.status = "format_failure";
        record.notes =
          attempt < MAX_FORMAT_ATTEMPTS
            ? "format failure; retrying once"
            : "format failure after retry";
      }
    } catch (error) {
    ...
    writeJson(
      join(runDir, resultFileName(item.promptId, item.modelId, item.rep)),
      record,
    );
    ...
    const line = `[${padded}/${total}] ${item.modelId.padEnd(20)} ${item.promptId} rep=${item.rep} ${record.status.padEnd(18)} ${(
...
async function loadPromptTemplate(): Promise<string> {
  ...
  if (!existsSync(ROUTING_PROMPT_TEMPLATE)) {
    throw new Error(
      `Routing prompt template missing: ${ROUTING_PROMPT_TEMPLATE}`,
    );
  }
  ...
  for (const entry of readdirSync(runDir)) {
    ...
    try {
      const parsed = JSON.parse(
        readFileSync(join(runDir, entry), "utf-8"),
      ) as RouterRunRecord;
      if (
        parsed &&
        parsed.prompt_id &&
        parsed.model_id !== undefined &&
        parsed.rep !== undefined
      ) {
...
function summarize(
  results: RouterRunRecord[],
  prompts: RouterPrompt[],
): Record<string, unknown> {
  const perModel: Record<
    string,
    { total: number; format_failures: number; top1: number; top3: number }
  > = {};
  for (const r of results) {
    const bucket = perModel[r.model_id] ?? {
      total: 0,
      format_failures: 0,
      top1: 0,
      top3: 0,
