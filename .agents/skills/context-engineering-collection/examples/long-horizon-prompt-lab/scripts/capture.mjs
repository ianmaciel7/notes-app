...
async function main() {
  ...
  await page.setViewport({
    width: WIDTH,
    height: 1000,
    deviceScaleFactor: FULL_DPR,
  });
  await page.goto(overview, { waitUntil: "networkidle0" });
  await page.screenshot({
    path: resolve(OUT, "shot-overview-full.png"),
    fullPage: true,
  });
  ...
  const ids = await page.evaluate(() =>
    window.PROMPT_LAB_DATA.pairs.map((p) => p.id),
  );
  ...
  for (const id of ids) {
    ...
    const expected = await page.evaluate(
      (pid) => window.PROMPT_LAB_DATA.pairs.find((p) => p.id === pid).title,
      id,
    );
    const rendered = await page.$eval(".pair-head h2", (n) => n.textContent);
    if (rendered !== expected) {
      throw new Error(
        `Rendered pair "${rendered}" != expected "${expected}" for id ${id}`,
      );
    }
    await clipElement(
      page,
      "#pair-split",
      resolve(OUT, `shot-${id}-split.png`),
      12,
    );
    await clipElement(
      page,
      "#scorecard",
      resolve(OUT, `shot-${id}-scorecard.png`),
      12,
    );

    await load(id, FULL_DPR);
    await page.screenshot({
      path: resolve(OUT, `shot-${id}-full.png`),
      fullPage: true,
    });
