...
async function main() {
  ...
  cpSync(
    resolve(LAB, "screenshots", "shot-hero.png"),
    resolve(site, "social-card.png"),
  );
  ...
  try {
    ...
    for (const width of [320, 390, 768, 1480]) {
      ...
      for (const path of pages) {
        ...
        if (metrics.h1 !== 1)
          failures.push(`${path}@${width}: ${metrics.h1} h1 elements`);
        if (metrics.nav !== 4)
          failures.push(`${path}@${width}: ${metrics.nav} nav links`);
    ...
    await page.waitForFunction(
      () =>
        document
          .querySelector("#template-status")
          .textContent.includes("Copied"),
      { timeout: 3000 },
    );

    await page.goto(origin + "lab.html#ml-optimization", {
      waitUntil: "networkidle0",
    });
    ...
    if (
      prompts.some((prompt) => prompt.scrollHeight > prompt.clientHeight + 1)
    ) {
      failures.push("lab: prompt content clipped");
    }
    if (prompts.map((prompt) => prompt.words).join(",") !== "183,836") {
      failures.push(
        `lab: unexpected prompt word counts ${JSON.stringify(prompts)}`,
      );
    }
    ...
    if (selected !== "security-audit")
      failures.push(`lab: keyboard selected ${selected}`);
    ...
    if (!downloadResponse.ok())
      failures.push(`prompt download: HTTP ${downloadResponse.status()}`);

    await page.goto(origin + "references.html", { waitUntil: "networkidle0" });
    const referenceCount = await page.$$eval(
      ".reference-card",
      (nodes) => nodes.length,
    );
    if (referenceCount < 25)
      failures.push(`references: only ${referenceCount} cards`);
    ...
    if (noScriptDownloads !== 8) {
      failures.push(
        `lab no-JS fallback: ${noScriptDownloads} prompt downloads`,
      );
    }
