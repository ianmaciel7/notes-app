...
(function () {
  ...
  if (!DATA) {
    document.body.innerHTML =
      "<p style='padding:40px;font-family:sans-serif'>Missing data.js. Run scripts/build_lab.py first.</p>";
    return;
  }
  ...
  function normalizePromptDisplay(text) {
    ...
    for (const raw of lines) {
      ...
      const isHeader =
        /^[A-Z][A-Z0-9 /&().-]{2,}$/.test(trimmed) && trimmed.length <= 64;
      const isFormula =
        /^(cost\(|E_r\[|mean\(|    )/.test(raw) ||
        /^[ ]{2,}cost\(|^[ ]{2,}E_r\[/.test(raw);
  ...
  function promptPanel(kind, pair) {
    ...
    h.appendChild(
      el(
        "span",
        "panel-tag",
        isBefore ? "01 / Original prompt" : "02 / Optimized prompt",
      ),
    );
    ...
    panel.appendChild(
      el(
        ...
        isBefore
          ...
          : "Ready to paste into a long-running agent",
      ),
    );

    if (isBefore) {
      const tech = el("div", "techniques");
      pair.before_techniques.forEach((t) =>
      ...
      [
        ...
        "return gate",
        "harness boundaries",
      ].forEach((t) => tech.appendChild(el("span", "technique", t)));
      panel.appendChild(tech);
    }
  ...
  function scorecard(pair) {
    ...
    const caption = el(
      "caption",
      "visually-hidden",
      "Pre-launch structural audit comparing original and optimized prompts",
    );
    table.appendChild(caption);
    const thead = el("thead");
    const hr = el("tr");
    ["Rubric dimension (2 = adversary-proof)", "Before", "After"].forEach(
      (h, i) => {
    ...
    ["before", "after"].forEach((k) => {
      totalTr.appendChild(
        el("td", "cell-score", scoreLine(pair.score_summary[k])),
      );
    });
