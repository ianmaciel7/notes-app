...
async function main() {
  ...
  const criteria = [
    {
      ...
      description: "Correctness of biological/medical information",
    },
    { name: "Completeness", description: "Covers the key steps and concepts" },
    {
      name: "Accessibility",
      description: "Understandable by general audience",
    },
  ];

  const rubrics = await Promise.all(
    criteria.map((c) =>
      agent.generateRubric({
  ...
  const scoreResult = await agent.score({
    ...
    rubric: {
      ...
      levelDescriptions: rubrics[0].success
        ? Object.fromEntries(
            rubrics[0].levels.map((l) => [String(l.score), l.label]),
          )
        : undefined,
    },
  });
  ...
  if (comparisonResult.success) {
    console.log(
      `Comparison Result: Response ${comparisonResult.winner} is better`,
    );
    console.log(
      `Confidence: ${(comparisonResult.confidence * 100).toFixed(0)}%`,
    );
    console.log("\nKey differences:");
    comparisonResult.differentiators
      .slice(0, 3)
  ...
  console.log(
    `Final score: ${scoreResult.success ? scoreResult.overallScore : "N/A"}`,
  );
  console.log(
    `Better response: ${comparisonResult.success ? comparisonResult.winner : "N/A"}`,
  );
}
