...
describe("LLM Evaluator Skill Tests", () => {
    ...
    it("should handle multiple weighted criteria", async () => {
      const result = await agent.score({
        ...
        criteria: [
          {
            name: "Accuracy",
            description: "Scientific correctness",
            weight: 0.5,
          },
          {
            name: "Completeness",
  ...
  describe("Rubric Generation Skill", () => {
    it("should generate domain-specific rubrics", async () => {
      ...
      const allText = result.levels
        .map((l) => l.description + l.characteristics.join(" "))
        .join(" ");
      expect(allText.toLowerCase()).toMatch(
        /variable|function|comment|name|structure|code|read/i,
      );
    }, 60000);
  ...
  describe("Context Fundamentals Skill Application", () => {
    it("should utilize provided context in evaluation", async () => {
      ...
      const result = await agent.score({
        response:
          "Combining SSRIs with MAOIs can lead to serotonin syndrome, a potentially life-threatening condition.",
        ...
        criteria: [
          { name: "Accuracy", description: "Medical accuracy", weight: 0.5 },
          {
            ...
            description: "Appropriate for audience",
            weight: 0.5,
          },
        ],
      });
