...
function verifyEnforcedRows(text) {
  const section =
    text.split("## Enforced with numbers")[1]?.split(/^## /m)[0] || "";
  const rows = section
    .split("\n")
    .filter((line) => /^\s*\|/.test(line) && !/^\s*\|[-| :]+\s*$/.test(line));
  if (rows.length < 2)
    return [fail("enforced-rows", "Enforced with numbers has no data rows.")];
...
function verifyExceptions(text) {
  ...
  if (/\bNone\.?\s*$/im.test(section.trim()))
    return [ok("exceptions", "No active exceptions are declared.")];
  const rows = section
    .split("\n")
    .filter((line) => /^\s*\|/.test(line) && !/^\s*\|[-| :]+\s*$/.test(line));
  if (rows.length < 2)
...
function verifyConstraintCommands(repoRoot, text) {
  ...
  return results.length
    ? results
    : [skip("constraint-commands", "No pnpm package scripts cited.")];
}
