...
function getRealComponentCount(repoRoot) {
  ...
  return {
    count: allTsx.filter((f) => !f.endsWith(".stories.tsx")).length,
    totalTsxFiles: allTsx.length,
  };
}
...
function findComponentCountMentions(text) {
  ...
  while ((m = keywordRe.exec(text)) !== null) {
    ...
    const line = text
      ...
      .trim();
    // Skip bare YAML key lines (e.g. "components:" or "  button-default:") —
    // those are frontmatter structure, not English prose component-count claims.
    if (/^[\w-]+:\s*$/.test(line) || /^\s{2,}[\w-]+:\s*$/.test(line)) continue;
    mentions.push({ number, line });
  }
  return mentions;
}
...
function verifyNOfMClaim(repoRoot, text, { label, keyword, countRealFn }) {
  ...
  if (!m)
    return [
      skip(label, `No "N of M ${keyword}" claim found — nothing to check.`),
    ];
  const claimedN = parseInt(m[1], 10);
  const claimedM = parseInt(m[2], 10);
  const real = countRealFn(repoRoot);
  if (real == null)
    return [skip(label, "Could not compute the real count — skipping.")];
  return [
    real.total === claimedM
      ? ok(
          label,
...
function verifyLocalMdLinks(repoRoot, filePath, text) {
  ...
  if (results.length === 0)
    results.push(skip("doc-links", "No local .md links found."));
  return results;
}
...
function verifyCitedPackageScripts(repoRoot, text) {
  ...
  if (!fs.existsSync(pkgPath))
    return [skip("package-scripts", "No package.json at repo root.")];
  let pkg;
  try {
    pkg = JSON.parse(fs.readFileSync(pkgPath, "utf8"));
  } catch (e) {
    return [
      skip("package-scripts", `Could not parse package.json: ${e.message}`),
    ];
  }
  const scripts = new Set(Object.keys(pkg.scripts || {}));
  const re =
    /`(?:pnpm|npm run|npm|yarn)\s+([a-zA-Z][\w:-]*)`|(?:^|\n)\s*(?:pnpm|npm run|npm|yarn)\s+([a-zA-Z][\w:-]*)/g;
  const results = [];
  const seen = new Set();
  let m;
  ...
  if (results.length === 0)
    results.push(skip("package-scripts", "No pnpm/npm/yarn commands cited."));
  return results;
}
...
const SCAFFOLD_SIGNATURES = [
  {
    ...
    phrase: "bootstrapped with [`create-next-app`]",
  },
  ...
  { scaffold: "create-next-app", phrase: "Deploy on Vercel" },
  {
    ...
    phrase: "This project was bootstrapped with [Create React App]",
  },
  { scaffold: "vite", phrase: "npm create vite@latest" },
  {
    scaffold: "create-vue",
    phrase: "This template should help get you started developing with Vue",
  },
];
...
function getDesignFrontmatter(text) {
  // Normalise CRLF so Windows-encoded files are handled identically to LF files.
  const normalised = text.replace(/\r\n/g, "\n");
  if (!normalised.startsWith("---\n")) return null;
  const end = normalised.indexOf("\n---\n", 4);
  return end < 0 ? null : normalised.slice(4, end);
}
...
function verifyDesignDimensionValues(text) {
  const fm = getDesignFrontmatter(text);
  if (fm === null)
    return [skip("design-dimensions", "No frontmatter to inspect.")];
  const invalid = [];
  let section = null;
  for (const line of fm.split("\n")) {
...
function verifyAgentsGeneratedBlock(repoRoot) {
  ...
  if (!fs.existsSync(agentsMdPath))
    return [skip("generated-block", "No AGENTS.md at repo root.")];
...
function verifyAgentsCitedPaths(repoRoot, text) {
  ...
  if (results.length === 0)
    results.push(skip("agents-paths", "No local Markdown/JSON paths cited."));
  return results;
}
...
function verifyEnvHandling(repoRoot) {
  const envFiles = fs.existsSync(repoRoot)
    ? fs.readdirSync(repoRoot).filter((f) => /^\.env/.test(f))
    : [];
  if (envFiles.length === 0)
    return [ok("env-files", "No .env* files present at repo root.")];
  ...
  for (const f of envFiles) {
    try {
      ...
      if (e.status === 1)
        uncovered.push(f); // git ran fine, file just isn't ignored
...
function verifyCiExistence(repoRoot, expectNoCi) {
  ...
  const exists =
    fs.existsSync(workflowsDir) && fs.readdirSync(workflowsDir).length > 0;
  if (expectNoCi === null)
    return [
      exists
        ? ok("ci", ".github/workflows/ exists with workflow file(s).")
        : ok("ci", "No .github/workflows/ — no CI configured."),
    ];
  ...
  if (!expectNoCi && !exists)
    return [
      fail("ci", "Doc assumes CI exists, but no .github/workflows/ found."),
    ];
...
function verifyCiClaims(repoRoot, text) {
  ...
  const claimsCi =
    ...
    /\.github\/workflows\/[\w.-]+\.ya?ml/i.test(text);
...
function verifyJscpdThreshold(repoRoot, text) {
  ...
  if (!fs.existsSync(configPath))
    return [skip("jscpd-threshold", "No .jscpd.json found.")];
...
function verifyVersionClaims(repoRoot, text) {
  const pkgPath = path.join(repoRoot, "package.json");
  if (!fs.existsSync(pkgPath))
    return [skip("version-claims", "No package.json at repo root.")];
...
function verifyCitedFilesExist(repoRoot, text) {
  ...
  while ((m = fileRe.exec(text)) !== null) {
    ...
    const found = cited.includes("/")
      ...
      : fs.existsSync(path.join(repoRoot, "src", "components", "ui", cited)) ||
        fs.existsSync(path.join(repoRoot, "src", "app", cited));
    results.push(
      found
...
function verifyConventionalCommits(repoRoot, n = 10) {
  ...
  let log;
  try {
    log = execFileSync("git", ["log", `-${n}`, "--pretty=%s"], {
      cwd: repoRoot,
      encoding: "utf8",
    });
  } catch {
  ...
  if (subjects.length === 0)
    return [skip("conventional-commits", "No commits found.")];
...
function verifyPackageManagerPin(repoRoot, text) {
  const pkgPath = path.join(repoRoot, "package.json");
  if (!fs.existsSync(pkgPath))
    return [skip("package-manager-pin", "No package.json at repo root.")];
  const pkg = JSON.parse(fs.readFileSync(pkgPath, "utf8"));
