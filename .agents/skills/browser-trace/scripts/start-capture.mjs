...
import { ensureDir, isAlive, isoUtcSeconds, runDir, sleepMs, writeJson } from "./lib.mjs";
...
const domainsList = (process.env.O11Y_DOMAINS || "Network Console Runtime Log Page")
  .trim()
  .split(/\s+/);
...
if (!isAlive(cdp.pid)) {
  ...
  try {
    console.error(fs.readFileSync(path.join(RD, "cdp", "stderr.log"), "utf8"));
  } catch {}
  try {
