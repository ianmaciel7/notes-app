...
const anchorCdp = events.map((e) => e?.params?.timestamp).find(isMonotonic) ?? null;
...
for (const [bucket, predicate] of BUCKETS) {
  const matched = events.filter((e) => predicate(e.method ?? "")).map(stripPid);
...
for (const [thisPid, pageEvents] of [...pageMap.entries()].sort((a, b) => a[0] - b[0])) {
  ...
  for (const [bucket, predicate] of BUCKETS) {
    const matched = pageEvents.filter((e) => predicate(e.method ?? "")).map(stripPid);
...
function computePageSummary(pid, url, pageEvents) {
  const ts = pageEvents.map((e) => e?.params?.timestamp).filter(isMonotonic);
