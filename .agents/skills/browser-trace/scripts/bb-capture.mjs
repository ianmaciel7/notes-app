...
import { readJson, runCmd, runDir, writeJson } from "./lib.mjs";
...
if (target === "--new") {
  ...
  if (!r.ok) {
    console.error(r.stderr || "browse cloud sessions create failed");
    process.exit(1);
  }
  ...
  if (!r.ok) {
    console.error(r.stderr || "browse cloud sessions get failed");
    process.exit(1);
  }
  sessionJson = JSON.parse(r.stdout);
  if (sessionJson.status !== "RUNNING") {
    console.error(
      `Session ${target} is not RUNNING (status=${sessionJson.status}). Recreate with --keep-alive.`,
    );
    process.exit(1);
  }
}
...
if (dbg.ok) {
  try {
    debugJson = JSON.parse(dbg.stdout);
  } catch {}
}
...
if (!runId) {
  console.error("could not parse run_id from start-capture output");
  process.exit(1);
}
