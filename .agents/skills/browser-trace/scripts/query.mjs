...
import { isTopNav, readJson, readJsonl, runDir } from "./lib.mjs";
...
function cmdList() {
  ...
  if (!summary) {
    console.error("no summary.json — run bisect-cdp.mjs first");
    process.exit(1);
  }
...
function cmdPage(pidArg, bucketArg) {
  if (pidArg === undefined) {
    console.error("page id required");
    process.exit(2);
  }
  const pdir = pageDir(pidArg);
  if (!fs.existsSync(pdir)) {
    console.error(`no such page: ${pidArg}`);
    process.exit(1);
  }
  ...
  if (!bucketArg) {
    ...
    if (!s) {
      console.error(`no summary.json for page ${pidArg}`);
      process.exit(1);
    }
    console.log(JSON.stringify(s, null, 2));
    return;
  ...
  if (!fs.existsSync(file)) {
    console.error(`(empty: ${bucketArg} for page ${pidArg})`);
    return;
  }
