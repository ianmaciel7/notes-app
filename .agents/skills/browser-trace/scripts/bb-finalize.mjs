...
import { ensureDir, readJson, runCmd, runDir } from "./lib.mjs";
...
if (!sessionId) {
  console.error(
    "manifest has no .browserbase.session_id — was this run captured via bb-capture.mjs?",
  );
  process.exit(1);
}
...
// Server-side logs. Often empty — the firehose in cdp/raw.ndjson is the source of truth.
{
  ...
  if (r.ok) {
    ...
    let n = "?";
    try {
      n = String(JSON.parse(r.stdout).length ?? "?");
    } catch {}
