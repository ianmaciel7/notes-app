...
- Read `.ladle/config.mjs` for the actual story globs, default route, root order, and navigation-only capitalization. `.ladle/components.tsx` is the global visual/i18n provider and imports the application CSS. Do not duplicate those providers or maintain stale configuration copies in this skill.
- Tailwind v4 is wired through `postcss.config.mjs` with `@tailwindcss/postcss`.

## Authoring Rules
- Follow `docs/ladle.md`, also visible under **Docs / Ladle**. Keep the roots **Components**, **UI**, and **Docs**, explicit static titles in Title Case, short scenario names, and shallow area-based groups. Do not register identical scenarios twice.
- Keep stories beside components under `src/**/*.stories.tsx`; export functions typed with `Story<Props>`. Use `StoryDefault<Props>` with an explicit static `title`; scope controls to the stories that actually use them.
...
## Verification
- Run `node --test scripts/quality/ladle-catalog.test.mjs` to validate real Ladle metadata and catalog placement. Do not replace this with a filename-only check.
