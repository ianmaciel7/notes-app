...
| Truthful unavailable states | CONFIRMED: a visible unsupported command must state unavailability instead of showing an instructional placeholder. | Atomic-note commands assert named rendered outcomes; unsupported authenticated-reference transitions were not safely available. | Local pass; reference unavailable transitions not tested | `Atomic note object-type commands render named outcomes`; task 7.9 source guard rejects selector fallbacks. |
