...
2. **Add translations to all catalogs**: update `src/app/[lang]/dictionaries/en.json`, `src/app/[lang]/dictionaries/pt-BR.json`, and the fallback messages in `src/lib/i18n/messages.ts`.
...
5. **In Firebase Auth UI**: use `@firebase-oss/ui-core`'s `getTranslation(ui, ...)` for Firebase-owned labels/prompts, and `getAuthErrorMessage(error, locale)` from `src/lib/i18n` for normalized authentication error handling.
